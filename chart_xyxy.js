...
SC.ChartXYXY.prototype.resetZoomAndPan = function () {
    ...
    var bx = 0.04 * (this.constraints.maxx - this.constraints.minx),
...
SC.ChartXYXY.prototype.realToCanvas = function (aX, aY, aRoundTo05) {
    ...
    var w = this.canvas.width,
        ...
        y = h - h * (aY - this.miny) / (this.maxy - this.miny);
    if (aRoundTo05) {
        x = Math.floor(x) + 0.5;
        y = Math.floor(y) + 0.5;
    }
...
SC.ChartXYXY.prototype.render = function () {
    // Render chart
    this.frames++;
    var i, s, ser, sd, x, y, p, a, b, x0, y0, m, strx, stry, ofs;
    ...
    this.canvas.context.fillText('I(mA)', p.x + 2, 2);
    ...
    if (SC.showChartCursor && this.cursor) {
        ...
        if (this.cursor.y >= this.miny && this.cursor.y <= this.maxy) {
